...
class _SeasonOfKDEState extends State<SeasonOfKDE> {
  ...
  List<int> yearList = [2024, 2023, 2022];

  List<SokdeProjectModal> projectList = [];
  late Future<void> getProjectFunction;

  Future<void> initializeProjectLists() async {
    final data = await FirestoreService().getSokdeProjects(selectedYear);
    setState(() {
      projectList = data;
    });
  }
  ...
  Future<void> _onYearChanged(int year) async {
    ...
    final data = await FirestoreService().getSokdeProjects(year);
    setState(() {
      projectList = data;
    });
  }
